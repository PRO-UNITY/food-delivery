...
const AddDeliverToKitchen = () => {
  ...
  useEffect(() => {
    getUserData(`/delivery/create_kitchen_delivery/${id}`).then((res) =>
      setDelivery(res.no_active)
    );
  }, []);
  ...
  const handleAddDeliver = () => {
    ...
    EditWithFormData(`/delivery/create_kitchen_delivery/${id}`, formData).then(
      (res) => navigate("/home/main")
    );
  };
